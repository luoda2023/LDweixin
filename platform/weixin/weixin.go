...
// tryParseVoiceCommand converts natural-language voice commands to LDweixin commands.
func tryParseVoiceCommand(text string) string {
	lower := strings.ToLower(strings.TrimSpace(text))
	for _, prefix := range []string{"打开对话","切换到对话","切换对话","打开第","第","打开","切换到"} {
		if strings.HasPrefix(lower, prefix) {
			remain := strings.TrimSpace(lower[len(prefix):])
			for _, word := range strings.Fields(remain) {
				cleaned := strings.TrimRight(word, "个.")
				numWords := map[string]int{"一":1,"二":2,"三":3,"四":4,"五":5,"六":6,"七":7,"八":8,"九":9,"十":10}
				n, ok := numWords[cleaned]
				if !ok { fmt.Sscanf(cleaned, "%d", &n); ok = n > 0 }
				if ok && n > 0 && n <= 99 { return fmt.Sprintf("/switch %d", n) }
			}
		}
	}
	for _, p := range []string{"新建对话","新建会话","新对话","新会话"} { if strings.HasPrefix(lower, p) { return "/new" } }
	for _, p := range []string{"列出对话","会话列表","所有对话","列表"} { if strings.HasPrefix(lower, p) { return "/list" } }
	for _, p := range []string{"帮助","命令","菜单","怎么用"} { if strings.HasPrefix(lower, p) { return "/help" } }
	return text
}
...
func (p *Platform) sendChunkWithRetry(ctx context.Context, rc *replyContext, chunk string, chunkIdx, totalChunks int) error {
	...
	for attempt := 0; attempt < weixinSendMaxRetries; attempt++ {
		...
		err := p.api.sendText(ctx, rc.peerUserID, stripped, rc.contextToken, clientID)
		if err == nil {
			return nil
		}
		lastErr = err
