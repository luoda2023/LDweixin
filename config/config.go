...
type Config struct {
	...
	// Security controls security-related settings like direct IP access.
	Security SecurityConfig `toml:"security,omitempty"`
}

// SecurityConfig controls direct IP access settings.
type SecurityConfig struct {
	AllowDirectIPAccess *bool  `toml:"allow_direct_ip_access,omitempty" json:"allow_direct_ip_access"`
	PublicIP            string `toml:"public_ip,omitempty" json:"public_ip"`
}
...
type InstantReplyConfig struct {
	Enabled *bool  `toml:"enabled"` // default false
	Content string `toml:"content"` // custom reply text; empty = use i18n default ("â  Processing...")
}
...
type TTSConfig struct {
	...
	Voice        string                    `toml:"voice"`         // default voice name (for edge: "zh-CN-XiaoxiaoNeural"; for pico: "zh-CN"; for espeak: "zh"; for mimo: "mimo_default" / "å  ç  " / "Mia" â  
...
type ProjectConfig struct {
	...
	// by default â  the target user's login PATH is used. Dangerous variables
	...
	// Subordinate to ReplyFooter â  the master footer toggle.
...
// AliasConfig maps a trigger string to a command (e.g. "å  å  " â  "/help").
type AliasConfig struct {
	Name    string `toml:"name"`    // trigger text (e.g. "å  å  ")
...
// NOT validate â  callers must call validate() or validatePermissive() themselves.
...
//  3. Legacy quiet = true (without display.mode) â  "quiet".
//  4. Default â  "full".
//
...
//  3. mode-derived default (compact/quiet â  false, full â  true)
...
func validateUsersConfig(prefix string, u *UsersConfig) error {
	...
	wildcardCount := 0
	seenUserIDs := make(map[string]string) // userID â  role name
...
// â  â   Global provider CRUD â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  â  
...
type WeixinCredentialUpdateOptions struct {
	...
	AccountID         string // optional ilink_bot_id â  options.account_id
...
func GetGlobalSettings() map[string]any {
	...
	result := map[string]any{
		...
		"allow_direct_ip_access": cfg.Security.AllowDirectIPAccess != nil && *cfg.Security.AllowDirectIPAccess,
		"public_ip":              cfg.Security.PublicIP,
	}
...
type GlobalSettingsUpdate struct {
	...
	AllowDirectIPAccess *bool   `json:"allow_direct_ip_access"`
...
func SaveGlobalSettings(u GlobalSettingsUpdate) error {
	...
	if u.AllowDirectIPAccess != nil {
		cfg.Security.AllowDirectIPAccess = u.AllowDirectIPAccess
	}
	if u.PublicIP != nil {
		cfg.Security.PublicIP = *u.PublicIP
	}
