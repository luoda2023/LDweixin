...
func (e *Engine) applySessionFilter(sessions []AgentSessionInfo, agentName string, sm *SessionManager) []AgentSessionInfo {
...
func (e *Engine) cmdList(p Platform, msg *Message, args []string) {
	...
	if !supportsCards(p) {
		...
		agentSessions = e.applySessionFilter(agentSessions, agent.Name(), sessions)
...
func (e *Engine) formatToolResultEventFallback(toolName, result, status string, exitCode *int, success *bool) string {
	...
	if strings.TrimSpace(status) != "" {
		prefix += " " + strings.TrimSpace(status)
	} else if success != nil {
		if *success {
			prefix += " ok"
		} else {
