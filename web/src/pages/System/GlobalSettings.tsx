  ...
  const handleSave = async () => {
    ...
    try {
      await updateGlobalSettings({
        ...
        allow_direct_ip_access: allowDirectIPAccess,
      });
      ...
      <Card>
        ...
        <div className="space-y-4 max-w-lg">
          ...
          />
            ...
            {/* Security */}
      <Card>
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">{'settings.security'}</h3>
        <div className="space-y-4 max-w-lg">
          <Toggle
            label={t('settings.allowDirectIPAccess', 'Allow direct IP access')}
            value={allowDirectIPAccess}
            onChange={setAllowDirectIPAccess}
            hint={t('settings.allowDirectIPAccessHint', 'Allow clients to connect directly via IP address')}
          />
          <div>
            <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{t('settings.publicIP', 'Public IP')}</label>
            <p className="text-[11px] text-gray-400 mb-1.5">{t('settings.publicIPHint', 'Your relay server public IP. Double-click to copy.')}</p>
            <div className="relative">
              <input
                ...
                readOnly
                className="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white cursor-pointer select-all"
                placeholder={t('settings.publicIPPlaceholder', 'Relay server auto-detected IP')}
              />
              {copied ? (
                <Check size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" />
              ) : (
                <Copy size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
              )}
            </div>
          </div>
        </div>
